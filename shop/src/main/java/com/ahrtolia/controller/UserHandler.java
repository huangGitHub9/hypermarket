...
public class UserHandler {
    ...
    public String updateOrder(int addressId, HttpSession session, int zfmoney) throws Exception {
        int userId = ((User) session.getAttribute("user")).getId();
        ...
        Map<String, String> result = dataJoinUtils.wxPay("http://106.15.196.6:80/main/getAll.action", serialNumber, "1", "106.15.196.6", "海玩商品");
        //将二维码地址封装至order对象中
    ...
    public String doActive(String activeCode,HttpSession session){
        if (activeCode == "0"){
            session.setAttribute("codeError","激活码失效,请重新认证！");
        }else{
        ...
        if (code != null) {
            ...
            if (access != null) {
                ...
                if (user == null) {
                    ...
                    List<Order> orderCar = userService.getMyProductCar(user.getId());
                    session.setAttribute("orders", orderCar);
                    int money = 0;
                    for (Order o : orderCar) {
                        money += o.getCount() * o.getProduct().getPrice();
