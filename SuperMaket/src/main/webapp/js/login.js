...
//手机号码输入框
var inputPhone = document.getElementsByName('userCode')[0];
//alert(inputPhone);
//密码输入框
...
inputPassword.onblur = function () {
    ...
    //*密码必须为6-16位<br/>*必须有字母、数字或特殊字符其中两种
    if (!reg.test(this.value)) {
        rcInnerVirity.style.display = "block";
        rcInnerVirityText.innerText = "密码为6-16位的字母或数字或特殊字符的结合";
        rcInnerNum.style.display = "none";
    } else {
        passFlag = true;
        return;
...
loginBtn.onclick = function (e) {
    ...
    if (inputPhone.value == "") {
        rcInnerNum.style.display = "block";
        rcinnerText.innerText = "账号或者密码不合格！";
        rcInnerVirity.style.display = "none";
        return;
    }
...
//获取手机号码输入框
var noPhoneNum = document.getElementsByName('phone')[1];
//console.log(noPhoneNum);
//获取密码输入框
var otp = document.getElementsByName('code')[1];
//获取手机动态密码按钮
