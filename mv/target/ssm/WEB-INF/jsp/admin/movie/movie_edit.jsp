		...
		<div style="width:600px ; margin: auto;">
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<form class="form-horizontal" role="form" action="<c:url value="/admin/movieEditA.action"/>"
						  method="post" enctype="multipart/form-data">
						<div class="form-group">
							<label  class="col-sm-2 control-label">视频名称:</label>
							<div class="col-sm-8">
								<input type="text" class="form-control"  placeholder="${requestScope.movie.movieName}"
									   name="movieName" value="${requestScope.movie.movieName}"
								>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control"   placeholder="${requestScope.movie.movieDirector}"
									name ="movieDirector" value="${requestScope.movie.movieDirector}"
								>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="layui-input form-control"  name="movieTime"  id="birthday" placeholder="<fmt:formatDate
											value='${requestScope.movie.movieTime}'
											type="date"
											dateStyle="medium"
						...
						<div class="form-group">
							...
							<div class="checkbox" name="leixng" style="width: 373px; margin: 0 auto;">
								<c:forEach items="${requestScope.movieTypes}" var="movieType">
									<label>
										<input type="checkbox"  name="movieTypeName"
												  value="${movieType.movieTypeId}"
													<c:forEach items="${requestScope.movie.movieTypes}" var="mT">
															<c:if test="${mT.movieTypeId eq movieType.movieTypeId}">
																checked= "checked"
															</c:if>
													</c:forEach>
							...
							<div style="float: left;margin-left: 15px;">
								<select class="form-control" name="movieGradeId" >
									...
									<c:forEach items="${requestScope.movieGradeList}" var="mg">
										...
										<c:if test="${mg.movieGradeId eq requestScope.movie.movieGrade.movieGradeId}">
											selected="selected"
										</c:if>
										>${mg.movieGradeName}
										</option>
						...
						<div class="form-group">
							...
							<div class="layui-input-block">
								<textarea placeholder="${requestScope.movie.movieProtagonist}" class="layui-textarea"
										name="movieProtagonist">${requestScope.movie.movieProtagonist}
								</textarea>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="layui-input-block">
								<textarea placeholder="${requestScope.movie.movieIntro}" class="layui-textarea"
										name="movieIntro">${requestScope.movie.movieIntro}
								</textarea>
							</div>
						</div>
